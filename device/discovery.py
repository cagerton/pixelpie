# This is the Dropbit discovery protocol wire format.
# Devices will broadcast this data in UDP packets...
...
class PixelPusher(ctypes.Structure):
    ...
    def sane(self):
        ...
        sane = True
        if self.strips_attached > 8 or self.strips_attached == 0:
            logging.error("Strip count out of range: %d", self.strips_attached)
            sane = False
        if self.pixels_per_strip == 0 or self.pixels_per_strip > 480:
            logging.error("Too many pixels per strip? %d", self.pixels_per_strip)
            sane = True
        return sane
